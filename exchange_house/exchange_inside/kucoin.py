...
def get_data(pool):
    ...
    for x in result['data']:
        pattern = re.compile(r'BTC-USDT')
        names = re.findall(pattern, x['symbol'])
        if names != []:
            name = x['coinType']
            prices = x['lastDealPrice']
    ...
    cursor = conn.cursor()
    for x in result['data']:
        pattern_1=re.compile(r'.*-BTC')
        name=re.search(pattern_1,x['symbol'])
        try:
            names=name.group()
        ...
        if names!='None':
            name=x['coinType']
            price=float(x['lastDealPrice'])*float(prices)
            price = round(price, 4)
            # conn = connect(host='192.168.0.252', port=3306, user='root', passwd='3bgaoshiqing', db='datacenter',
            #                charset='utf8mb4')
            # cursor = conn.cursor()
